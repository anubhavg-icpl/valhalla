...
/// kernel-init delivers a few elements (e.g. panic implementation) necessary
/// to run code in kernel. It is referenced here only to ensure it is linked in.
extern crate kernel_init;
...
use winapi::{
    km::wdm::{
        ...
        IoDeleteSymbolicLink, IoGetCurrentIrpStackLocation, _IO_STACK_LOCATION_READ,
        DEVICE_FLAGS::DO_DIRECT_IO, DEVICE_OBJECT, DEVICE_TYPE, DRIVER_OBJECT, IO_STACK_LOCATION,
        IRP, IRP_MJ, PDEVICE_OBJECT, PEPROCESS,
    },
    shared::{
...
/// Kernel-mode entry point invoked by the Windows I/O Manager when the driver
/// is loaded. Caller must guarantee `driver` and `_path` are valid kernel
/// pointers provided by the I/O Manager.
///
/// # Safety
///
/// This function dereferences raw pointers supplied by the kernel and calls
/// kernel-mode APIs. It must only be invoked by the Windows I/O Manager as the
/// driver entry point.
#[no_mangle]
